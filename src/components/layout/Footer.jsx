...
export default function Footer() {
  ...
  return (
    ...
    >
      ...
      {/* Modern 3D Floating Leaf Parallax System */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
        {[
          { size: 28, x: "10%", y: "20%", delay: 0, speed: 6, rot: 45 },
          { size: 40, x: "82%", y: "15%", delay: 2, speed: 7.5, rot: -20 },
          { size: 32, x: "48%", y: "55%", delay: 1, speed: 6.8, rot: 75 },
          { size: 24, x: "88%", y: "68%", delay: 3, speed: 8.5, rot: 110 },
          { size: 36, x: "18%", y: "72%", delay: 1.5, speed: 6.2, rot: -50 }
        ].map((leaf, idx) => (
          <motion.div
            key={idx}
            className="absolute"
            style={{
              left: leaf.x,
              top: leaf.y,
              x: mousePos.x * (idx * 25 + 10),
              y: mousePos.y * (idx * 25 + 10),
              transition: 'transform 0.15s cubic-bezier(0.25, 0.46, 0.45, 0.94)'
            }}
            animate={{
              y: [0, -15, 0],
              rotate: [leaf.rot, leaf.rot + 15, leaf.rot]
            }}
            transition={{
              duration: leaf.speed,
              repeat: Infinity,
          ...
          >
            <LeafIcon 
              className="opacity-[0.05] drop-shadow-[0_0_15px_#76C945]" 
